...
export class AvatarService {
  public constructor(private apiService: FileService) {
  }

  public getUrl(profile: User): Observable<string> {
    const key: number = profile?.id ?? 0;

    if (!profile || !profile.avatar_uploaded_url) {
      const avatar = createAvatar(pixelArt, { seed: profile.id ?? 'None' } as StyleOptions<unknown>);
